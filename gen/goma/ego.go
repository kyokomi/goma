// Generated by ego on Sat Jun  6 22:08:54 2015.
// DO NOT EDIT

package main
import (
"fmt"
"html"
"io"
)
//line templates/asset_file_template.go.ego:1
func AssetFileTemplate(w io.Writer, assetData AssetTemplateData) error  {
//line templates/asset_file_template.go.ego:1
_, _ = fmt.Fprint(w, "package  ")
//line templates/asset_file_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  assetData.DaoPkgName )))
//line templates/asset_file_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n\t\"io/ioutil\"\n)\n\n// AssetFile default read file\nfunc AssetFile(filePath string) ([]byte, error) {\n\treturn ioutil.ReadFile(filePath)\n}\n")
return nil
}
//line templates/asset_template.go.ego:1
func AssetTemplate(w io.Writer, assetData AssetTemplateData) error  {
//line templates/asset_template.go.ego:1
_, _ = fmt.Fprint(w, "package  ")
//line templates/asset_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  assetData.DaoPkgName )))
//line templates/asset_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n\t\"io/ioutil\"\n)\n\nfunc init() {\n\tsettings.sqlFile = true\n}\n\n// Asset default read file\nfunc Asset(filePath string) ([]byte, error) {\n\treturn ioutil.ReadFile(filePath)\n}\n")
return nil
}
//line templates/dao_simple_template.go.ego:1
func DaoSimpleTemplate(w io.Writer, daoData DaoTemplateData) error  {
//line templates/dao_simple_template.go.ego:1
_, _ = fmt.Fprint(w, "package  ")
//line templates/dao_simple_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.DaoPkgName )))
//line templates/dao_simple_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n    \"log\"\n    \n    \"database/sql\"\n    \n    \"")
//line templates/dao_simple_template.go.ego:12
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityImport )))
//line templates/dao_simple_template.go.ego:12
_, _ = fmt.Fprint(w, "\"\n)\n\n// ")
//line templates/dao_simple_template.go.ego:15
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:15
_, _ = fmt.Fprint(w, "Queryer is interface\ntype ")
//line templates/dao_simple_template.go.ego:16
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:16
_, _ = fmt.Fprint(w, "Queryer interface {\n\tQuery(query string, args ...interface{}) (*sql.Rows, error)\n\tExec(query string, args ...interface{}) (sql.Result, error)\n}\n\n// ")
//line templates/dao_simple_template.go.ego:21
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:21
_, _ = fmt.Fprint(w, " is generated ")
//line templates/dao_simple_template.go.ego:21
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:21
_, _ = fmt.Fprint(w, " table.\ntype ")
//line templates/dao_simple_template.go.ego:22
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:22
_, _ = fmt.Fprint(w, " struct {\n\t*sql.DB\n\tTableName string\n}\n\n// Query ")
//line templates/dao_simple_template.go.ego:27
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:27
_, _ = fmt.Fprint(w, " query\nfunc (g ")
//line templates/dao_simple_template.go.ego:28
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:28
_, _ = fmt.Fprint(w, ") Query(query string, args ...interface{}) (*sql.Rows, error) {\n\treturn g.DB.Query(query, args...)\n}\n\n// Exec ")
//line templates/dao_simple_template.go.ego:32
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:32
_, _ = fmt.Fprint(w, " exec\nfunc (g ")
//line templates/dao_simple_template.go.ego:33
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:33
_, _ = fmt.Fprint(w, ") Exec(query string, args ...interface{}) (sql.Result, error) {\n\treturn g.DB.Exec(query, args...)\n}\n\nvar _ ")
//line templates/dao_simple_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:37
_, _ = fmt.Fprint(w, "Queryer = (*")
//line templates/dao_simple_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:37
_, _ = fmt.Fprint(w, ")(nil)\n\n// ")
//line templates/dao_simple_template.go.ego:39
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:39
_, _ = fmt.Fprint(w, " is ")
//line templates/dao_simple_template.go.ego:39
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:39
_, _ = fmt.Fprint(w, ".\nfunc ")
//line templates/dao_simple_template.go.ego:40
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:40
_, _ = fmt.Fprint(w, "(db *sql.DB) ")
//line templates/dao_simple_template.go.ego:40
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:40
_, _ = fmt.Fprint(w, " {\n    tblDao := ")
//line templates/dao_simple_template.go.ego:41
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:41
_, _ = fmt.Fprint(w, "{}\n    tblDao.DB = db\n    tblDao.TableName = \"")
//line templates/dao_simple_template.go.ego:43
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:43
_, _ = fmt.Fprint(w, "\"\n\treturn tblDao\n}\n\n// Tx")
//line templates/dao_simple_template.go.ego:47
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:47
_, _ = fmt.Fprint(w, " is generated ")
//line templates/dao_simple_template.go.ego:47
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:47
_, _ = fmt.Fprint(w, " table transaction.\ntype Tx")
//line templates/dao_simple_template.go.ego:48
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:48
_, _ = fmt.Fprint(w, " struct {\n\t*sql.Tx\n\tTableName string\n}\n\n// Query Tx")
//line templates/dao_simple_template.go.ego:53
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:53
_, _ = fmt.Fprint(w, " query\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:54
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:54
_, _ = fmt.Fprint(w, ") Query(query string, args ...interface{}) (*sql.Rows, error) {\n\treturn g.Tx.Query(query, args...)\n}\n\n// Exec Tx")
//line templates/dao_simple_template.go.ego:58
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:58
_, _ = fmt.Fprint(w, " exec\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:59
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:59
_, _ = fmt.Fprint(w, ") Exec(query string, args ...interface{}) (sql.Result, error) {\n\treturn g.Tx.Exec(query, args...)\n}\n\nvar _ ")
//line templates/dao_simple_template.go.ego:63
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:63
_, _ = fmt.Fprint(w, "Queryer = (*Tx")
//line templates/dao_simple_template.go.ego:63
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:63
_, _ = fmt.Fprint(w, ")(nil)\n\n// Tx")
//line templates/dao_simple_template.go.ego:65
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:65
_, _ = fmt.Fprint(w, " is ")
//line templates/dao_simple_template.go.ego:65
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:65
_, _ = fmt.Fprint(w, ".\nfunc Tx")
//line templates/dao_simple_template.go.ego:66
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:66
_, _ = fmt.Fprint(w, "(tx *sql.Tx) Tx")
//line templates/dao_simple_template.go.ego:66
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:66
_, _ = fmt.Fprint(w, " {\n    tblDao := Tx")
//line templates/dao_simple_template.go.ego:67
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:67
_, _ = fmt.Fprint(w, "{}\n    tblDao.Tx = tx\n    tblDao.TableName = \"")
//line templates/dao_simple_template.go.ego:69
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:69
_, _ = fmt.Fprint(w, "\"\n\treturn tblDao\n}\n\n// SelectAll select ")
//line templates/dao_simple_template.go.ego:73
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:73
_, _ = fmt.Fprint(w, " table all recode.\nfunc (g ")
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, ") SelectAll() ([]")
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:74
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:75
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:75
_, _ = fmt.Fprint(w, "SelectAll(g)\n}\n\n// SelectAll transaction select ")
//line templates/dao_simple_template.go.ego:78
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:78
_, _ = fmt.Fprint(w, " table all recode.\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, ") SelectAll() ([]")
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:79
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:80
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:80
_, _ = fmt.Fprint(w, "SelectAll(g)\n}\n\nfunc _")
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, "SelectAll(g ")
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, "Queryer) ([]")
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:83
_, _ = fmt.Fprint(w, ", error) {\n    queryString := `\nselect\n  ")
//line templates/dao_simple_template.go.ego:86
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:86
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:86
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:86
 } 
//line templates/dao_simple_template.go.ego:86
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:87
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:87
 } 
//line templates/dao_simple_template.go.ego:87
_, _ = fmt.Fprint(w, "FROM\n  ")
//line templates/dao_simple_template.go.ego:88
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:88
_, _ = fmt.Fprint(w, "`\n\n\tvar es []")
//line templates/dao_simple_template.go.ego:90
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:90
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:90
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:91
_, _ = fmt.Fprint(w, "\n\trows, err := g.Query(queryString)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\n\tfor rows.Next() {\n\t\tvar e ")
//line templates/dao_simple_template.go.ego:97
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:97
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:97
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:98
_, _ = fmt.Fprint(w, "\n\t\tif err := e.Scan(rows); err != nil {\n\t\t\tbreak\n\t\t}\n\n\t\tes = append(es, e)\n\t}\n\tif err != nil {\n\t    log.Println(err, queryString)\n\t\treturn nil, err\n\t}\n\n\treturn es, nil\n}\n\n// SelectByID select ")
//line templates/dao_simple_template.go.ego:112
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:112
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc (g ")
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, ") SelectByID(")
//line templates/dao_simple_template.go.ego:113
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:113
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:113
 continue 
//line templates/dao_simple_template.go.ego:113
 } 
//line templates/dao_simple_template.go.ego:113
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:113
 } 
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:113
 } 
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:113
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:114
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:114
_, _ = fmt.Fprint(w, "SelectByID(g, ")
//line templates/dao_simple_template.go.ego:114
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:114
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:114
 continue 
//line templates/dao_simple_template.go.ego:114
 } 
//line templates/dao_simple_template.go.ego:114
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:114
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:114
 } 
//line templates/dao_simple_template.go.ego:114
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:114
 } 
//line templates/dao_simple_template.go.ego:114
_, _ = fmt.Fprint(w, ")\n}\n\n// SelectByID transaction select ")
//line templates/dao_simple_template.go.ego:117
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:117
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, ") SelectByID(")
//line templates/dao_simple_template.go.ego:118
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:118
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:118
 continue 
//line templates/dao_simple_template.go.ego:118
 } 
//line templates/dao_simple_template.go.ego:118
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:118
 } 
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:118
 } 
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:118
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:119
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:119
_, _ = fmt.Fprint(w, "SelectByID(g, ")
//line templates/dao_simple_template.go.ego:119
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:119
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:119
 continue 
//line templates/dao_simple_template.go.ego:119
 } 
//line templates/dao_simple_template.go.ego:119
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:119
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:119
 } 
//line templates/dao_simple_template.go.ego:119
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:119
 } 
//line templates/dao_simple_template.go.ego:119
_, _ = fmt.Fprint(w, ")\n}\n\nfunc _")
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, "SelectByID(g ")
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, "Queryer, ")
//line templates/dao_simple_template.go.ego:122
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:122
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:122
 continue 
//line templates/dao_simple_template.go.ego:122
 } 
//line templates/dao_simple_template.go.ego:122
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:122
 } 
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:122
 } 
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:122
_, _ = fmt.Fprint(w, ", error) {\n\tqueryString := `\nselect\n  ")
//line templates/dao_simple_template.go.ego:125
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:125
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:125
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:125
 } 
//line templates/dao_simple_template.go.ego:125
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:126
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:126
 } 
//line templates/dao_simple_template.go.ego:126
_, _ = fmt.Fprint(w, "FROM\n  ")
//line templates/dao_simple_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:128
_, _ = fmt.Fprint(w, "\nWHERE\n")
//line templates/dao_simple_template.go.ego:129
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:129
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:129
 continue 
//line templates/dao_simple_template.go.ego:129
 } 
//line templates/dao_simple_template.go.ego:129
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:129
_, _ = fmt.Fprint(w, "and")
//line templates/dao_simple_template.go.ego:129
 } 
//line templates/dao_simple_template.go.ego:129
_, _ = fmt.Fprint(w, "  ")
//line templates/dao_simple_template.go.ego:129
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:129
_, _ = fmt.Fprint(w, " = ")
//line templates/dao_simple_template.go.ego:129
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Placeholder(idx + 1) )))
//line templates/dao_simple_template.go.ego:130
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:130
 } 
//line templates/dao_simple_template.go.ego:130
_, _ = fmt.Fprint(w, "`\n\trows, err := g.Query(queryString,\n\t")
//line templates/dao_simple_template.go.ego:132
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:132
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:132
 continue 
//line templates/dao_simple_template.go.ego:132
 } 
//line templates/dao_simple_template.go.ego:132
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:132
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_simple_template.go.ego:133
 } 
//line templates/dao_simple_template.go.ego:133
_, _ = fmt.Fprint(w, ")\n\tif err != nil {\n\t\treturn ")
//line templates/dao_simple_template.go.ego:135
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:135
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:135
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:135
_, _ = fmt.Fprint(w, "{}, err\n\t}\n\tdefer rows.Close()\n\n\tif !rows.Next() {\n\t\treturn ")
//line templates/dao_simple_template.go.ego:140
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:140
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:140
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:140
_, _ = fmt.Fprint(w, "{}, sql.ErrNoRows\n\t}\n\n\tvar e ")
//line templates/dao_simple_template.go.ego:143
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:143
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:143
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:144
_, _ = fmt.Fprint(w, "\n\tif err := e.Scan(rows); err != nil {\n\t    log.Println(err, queryString)\n\t\treturn ")
//line templates/dao_simple_template.go.ego:146
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:146
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:146
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:146
_, _ = fmt.Fprint(w, "{}, err\n\t}\n\t\n\treturn e, nil\n}\n\n// Insert insert ")
//line templates/dao_simple_template.go.ego:152
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:152
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, ") Insert(e ")
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:153
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:154
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:154
_, _ = fmt.Fprint(w, "Insert(g, e)\n}\n\n// Insert transaction insert ")
//line templates/dao_simple_template.go.ego:157
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:157
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, ") Insert(e ")
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:158
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:159
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:159
_, _ = fmt.Fprint(w, "Insert(g, e)\n}\n\nfunc _")
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, "Insert(g ")
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, "Queryer, e ")
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:162
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\tqueryString := `\ninsert into ")
//line templates/dao_simple_template.go.ego:164
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:164
_, _ = fmt.Fprint(w, "(\n  ")
//line templates/dao_simple_template.go.ego:165
 var isComma bool 
//line templates/dao_simple_template.go.ego:165
 isComma = false 
//line templates/dao_simple_template.go.ego:165
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:165
 if !column.IsAutoIncrement { 
//line templates/dao_simple_template.go.ego:165
 if !isComma { 
//line templates/dao_simple_template.go.ego:165
 isComma = true 
//line templates/dao_simple_template.go.ego:165
 } else { 
//line templates/dao_simple_template.go.ego:165
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:165
 } 
//line templates/dao_simple_template.go.ego:165
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:165
 } 
//line templates/dao_simple_template.go.ego:166
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:166
 } 
//line templates/dao_simple_template.go.ego:166
_, _ = fmt.Fprint(w, ") values(\n  ")
//line templates/dao_simple_template.go.ego:167
 idx := 0 
//line templates/dao_simple_template.go.ego:167
 isComma = false 
//line templates/dao_simple_template.go.ego:167
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:167
 if !column.IsAutoIncrement { 
//line templates/dao_simple_template.go.ego:167
 if !isComma { 
//line templates/dao_simple_template.go.ego:167
 isComma = true 
//line templates/dao_simple_template.go.ego:167
 } else { 
//line templates/dao_simple_template.go.ego:167
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:167
 } 
//line templates/dao_simple_template.go.ego:167
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Placeholder(idx + 1) )))
//line templates/dao_simple_template.go.ego:167
  idx++ 
//line templates/dao_simple_template.go.ego:167
 } 
//line templates/dao_simple_template.go.ego:168
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:168
 } 
//line templates/dao_simple_template.go.ego:168
_, _ = fmt.Fprint(w, ")`\n\tresult, err := g.Exec(queryString,\n\t")
//line templates/dao_simple_template.go.ego:170
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:170
 if !column.IsAutoIncrement { 
//line templates/dao_simple_template.go.ego:170
_, _ = fmt.Fprint(w, "e.")
//line templates/dao_simple_template.go.ego:170
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/dao_simple_template.go.ego:170
_, _ = fmt.Fprint(w, ",")
//line templates/dao_simple_template.go.ego:170
 } 
//line templates/dao_simple_template.go.ego:171
_, _ = fmt.Fprint(w, "\n    ")
//line templates/dao_simple_template.go.ego:171
 } 
//line templates/dao_simple_template.go.ego:171
_, _ = fmt.Fprint(w, ")\n    if err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n\n// Update update ")
//line templates/dao_simple_template.go.ego:178
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:178
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, ") Update(e ")
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:179
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:180
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:180
_, _ = fmt.Fprint(w, "Update(g, e)\n}\n\n// Update transaction update ")
//line templates/dao_simple_template.go.ego:183
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:183
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, ") Update(e ")
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:184
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:185
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:185
_, _ = fmt.Fprint(w, "Update(g, e)\n}\n\n// Update update ")
//line templates/dao_simple_template.go.ego:188
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:188
_, _ = fmt.Fprint(w, " table.\nfunc _")
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, "Update(g ")
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, "Queryer, e ")
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, ".")
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_simple_template.go.ego:189
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\tqueryString := `\nupdate ")
//line templates/dao_simple_template.go.ego:191
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:191
_, _ = fmt.Fprint(w, " set\n  ")
//line templates/dao_simple_template.go.ego:192
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:192
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:192
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:192
 } 
//line templates/dao_simple_template.go.ego:192
_, _ = fmt.Fprint(w, "  ")
//line templates/dao_simple_template.go.ego:192
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:192
_, _ = fmt.Fprint(w, " = ")
//line templates/dao_simple_template.go.ego:192
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Placeholder(idx + 1) )))
//line templates/dao_simple_template.go.ego:193
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:193
 } 
//line templates/dao_simple_template.go.ego:193
_, _ = fmt.Fprint(w, " where\n  ")
//line templates/dao_simple_template.go.ego:194
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:194
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:194
 continue 
//line templates/dao_simple_template.go.ego:194
 } 
//line templates/dao_simple_template.go.ego:194
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:194
_, _ = fmt.Fprint(w, "and")
//line templates/dao_simple_template.go.ego:194
 } 
//line templates/dao_simple_template.go.ego:194
_, _ = fmt.Fprint(w, "  ")
//line templates/dao_simple_template.go.ego:194
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:194
_, _ = fmt.Fprint(w, " = ")
//line templates/dao_simple_template.go.ego:194
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Placeholder(idx + 1) )))
//line templates/dao_simple_template.go.ego:195
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:195
 } 
//line templates/dao_simple_template.go.ego:196
_, _ = fmt.Fprint(w, "\n`\n\tresult, err := g.Exec(queryString,\n\t")
//line templates/dao_simple_template.go.ego:198
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:198
_, _ = fmt.Fprint(w, "e.")
//line templates/dao_simple_template.go.ego:198
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/dao_simple_template.go.ego:198
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_simple_template.go.ego:199
 } 
//line templates/dao_simple_template.go.ego:200
_, _ = fmt.Fprint(w, "\n    ")
//line templates/dao_simple_template.go.ego:200
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:200
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:200
 continue 
//line templates/dao_simple_template.go.ego:200
 } 
//line templates/dao_simple_template.go.ego:200
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:200
_, _ = fmt.Fprint(w, "and")
//line templates/dao_simple_template.go.ego:200
 } 
//line templates/dao_simple_template.go.ego:200
_, _ = fmt.Fprint(w, "e.")
//line templates/dao_simple_template.go.ego:200
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/dao_simple_template.go.ego:200
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_simple_template.go.ego:201
 } 
//line templates/dao_simple_template.go.ego:201
_, _ = fmt.Fprint(w, ")\n\tif err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n\n// Delete delete ")
//line templates/dao_simple_template.go.ego:208
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:208
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, ") Delete(")
//line templates/dao_simple_template.go.ego:209
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:209
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:209
 continue 
//line templates/dao_simple_template.go.ego:209
 } 
//line templates/dao_simple_template.go.ego:209
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:209
 } 
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:209
 } 
//line templates/dao_simple_template.go.ego:209
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:210
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:210
_, _ = fmt.Fprint(w, "Delete(g, ")
//line templates/dao_simple_template.go.ego:210
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:210
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:210
 continue 
//line templates/dao_simple_template.go.ego:210
 } 
//line templates/dao_simple_template.go.ego:210
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:210
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:210
 } 
//line templates/dao_simple_template.go.ego:210
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:210
 } 
//line templates/dao_simple_template.go.ego:210
_, _ = fmt.Fprint(w, ")\n}\n\n// Delete transaction delete ")
//line templates/dao_simple_template.go.ego:213
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:213
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, ") Delete(")
//line templates/dao_simple_template.go.ego:214
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:214
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:214
 continue 
//line templates/dao_simple_template.go.ego:214
 } 
//line templates/dao_simple_template.go.ego:214
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:214
 } 
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:214
 } 
//line templates/dao_simple_template.go.ego:214
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_simple_template.go.ego:215
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:215
_, _ = fmt.Fprint(w, "Delete(g, ")
//line templates/dao_simple_template.go.ego:215
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:215
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:215
 continue 
//line templates/dao_simple_template.go.ego:215
 } 
//line templates/dao_simple_template.go.ego:215
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:215
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:215
 } 
//line templates/dao_simple_template.go.ego:215
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:215
 } 
//line templates/dao_simple_template.go.ego:215
_, _ = fmt.Fprint(w, ")\n}\n\n// Delete delete ")
//line templates/dao_simple_template.go.ego:218
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:218
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc _")
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, "Delete(g ")
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, "Queryer, ")
//line templates/dao_simple_template.go.ego:219
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:219
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:219
 continue 
//line templates/dao_simple_template.go.ego:219
 } 
//line templates/dao_simple_template.go.ego:219
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_simple_template.go.ego:219
 } 
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, " ")
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_simple_template.go.ego:219
 } 
//line templates/dao_simple_template.go.ego:219
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\tqueryString := `\ndelete\nfrom\n  ")
//line templates/dao_simple_template.go.ego:223
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_simple_template.go.ego:224
_, _ = fmt.Fprint(w, "\nwhere\n")
//line templates/dao_simple_template.go.ego:225
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:225
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:225
 continue 
//line templates/dao_simple_template.go.ego:225
 } 
//line templates/dao_simple_template.go.ego:225
 if idx != 0 { 
//line templates/dao_simple_template.go.ego:225
_, _ = fmt.Fprint(w, "and")
//line templates/dao_simple_template.go.ego:225
 } 
//line templates/dao_simple_template.go.ego:225
_, _ = fmt.Fprint(w, "  ")
//line templates/dao_simple_template.go.ego:225
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_simple_template.go.ego:225
_, _ = fmt.Fprint(w, " = ")
//line templates/dao_simple_template.go.ego:225
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Placeholder(idx + 1) )))
//line templates/dao_simple_template.go.ego:226
_, _ = fmt.Fprint(w, "\n")
//line templates/dao_simple_template.go.ego:226
 } 
//line templates/dao_simple_template.go.ego:227
_, _ = fmt.Fprint(w, "\n`\n    result, err := g.Exec(queryString,\n    ")
//line templates/dao_simple_template.go.ego:229
 for _, column := range daoData.Table.Columns { 
//line templates/dao_simple_template.go.ego:229
 if !column.IsPrimaryKey { 
//line templates/dao_simple_template.go.ego:229
 continue 
//line templates/dao_simple_template.go.ego:229
 } 
//line templates/dao_simple_template.go.ego:229
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_simple_template.go.ego:229
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_simple_template.go.ego:230
 } 
//line templates/dao_simple_template.go.ego:230
_, _ = fmt.Fprint(w, ")\n\tif err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n")
return nil
}
//line templates/dao_template.go.ego:1
func DaoTemplate(w io.Writer, daoData DaoTemplateData) error  {
//line templates/dao_template.go.ego:1
_, _ = fmt.Fprint(w, "package  ")
//line templates/dao_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.DaoPkgName )))
//line templates/dao_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n    \"log\"\n    \n    \"database/sql\"\n    \n    \"")
//line templates/dao_template.go.ego:12
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityImport )))
//line templates/dao_template.go.ego:12
_, _ = fmt.Fprint(w, "\"\n    \n    \"github.com/kyokomi/goma\"\n)\n\nvar table")
//line templates/dao_template.go.ego:17
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:17
_, _ = fmt.Fprint(w, " = \"")
//line templates/dao_template.go.ego:17
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:17
_, _ = fmt.Fprint(w, "\"\nvar columns")
//line templates/dao_template.go.ego:18
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:18
_, _ = fmt.Fprint(w, " = []string{\n")
//line templates/dao_template.go.ego:19
 for _, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:19
_, _ = fmt.Fprint(w, "\"")
//line templates/dao_template.go.ego:19
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_template.go.ego:19
_, _ = fmt.Fprint(w, "\",\n")
//line templates/dao_template.go.ego:20
 } 
//line templates/dao_template.go.ego:21
_, _ = fmt.Fprint(w, "\n}\n\n// ")
//line templates/dao_template.go.ego:23
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:23
_, _ = fmt.Fprint(w, "Queryer is interface\ntype ")
//line templates/dao_template.go.ego:24
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:24
_, _ = fmt.Fprint(w, "Queryer interface {\n\tQuery(query string, args ...interface{}) (*sql.Rows, error)\n\tExec(query string, args ...interface{}) (sql.Result, error)\n}\n\n// ")
//line templates/dao_template.go.ego:29
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:29
_, _ = fmt.Fprint(w, " is generated ")
//line templates/dao_template.go.ego:29
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:29
_, _ = fmt.Fprint(w, " table.\ntype ")
//line templates/dao_template.go.ego:30
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:30
_, _ = fmt.Fprint(w, " struct {\n\t*sql.DB\n\tTableName string\n\tColumns []string\n}\n\n// Query ")
//line templates/dao_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:36
_, _ = fmt.Fprint(w, " query\nfunc (g ")
//line templates/dao_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:37
_, _ = fmt.Fprint(w, ") Query(query string, args ...interface{}) (*sql.Rows, error) {\n\treturn g.DB.Query(query, args...)\n}\n\n// Exec ")
//line templates/dao_template.go.ego:41
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:41
_, _ = fmt.Fprint(w, " exec\nfunc (g ")
//line templates/dao_template.go.ego:42
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:42
_, _ = fmt.Fprint(w, ") Exec(query string, args ...interface{}) (sql.Result, error) {\n\treturn g.DB.Exec(query, args...)\n}\n\nvar _ ")
//line templates/dao_template.go.ego:46
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:46
_, _ = fmt.Fprint(w, "Queryer = (*")
//line templates/dao_template.go.ego:46
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:46
_, _ = fmt.Fprint(w, ")(nil)\n\n// ")
//line templates/dao_template.go.ego:48
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:48
_, _ = fmt.Fprint(w, " is ")
//line templates/dao_template.go.ego:48
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:48
_, _ = fmt.Fprint(w, ".\nfunc ")
//line templates/dao_template.go.ego:49
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:49
_, _ = fmt.Fprint(w, "(db *sql.DB) ")
//line templates/dao_template.go.ego:49
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:49
_, _ = fmt.Fprint(w, " {\n    tblDao := ")
//line templates/dao_template.go.ego:50
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:50
_, _ = fmt.Fprint(w, "{}\n    tblDao.DB = db\n    tblDao.TableName = table")
//line templates/dao_template.go.ego:52
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:53
_, _ = fmt.Fprint(w, "\n\ttblDao.Columns = columns")
//line templates/dao_template.go.ego:53
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:54
_, _ = fmt.Fprint(w, "\n\treturn tblDao\n}\n\n// Tx")
//line templates/dao_template.go.ego:57
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:57
_, _ = fmt.Fprint(w, " is generated ")
//line templates/dao_template.go.ego:57
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:57
_, _ = fmt.Fprint(w, " table transaction.\ntype Tx")
//line templates/dao_template.go.ego:58
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:58
_, _ = fmt.Fprint(w, " struct {\n\t*sql.Tx\n\tTableName string\n\tColumns []string\n}\n\n// Query Tx")
//line templates/dao_template.go.ego:64
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:64
_, _ = fmt.Fprint(w, " query\nfunc (g Tx")
//line templates/dao_template.go.ego:65
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:65
_, _ = fmt.Fprint(w, ") Query(query string, args ...interface{}) (*sql.Rows, error) {\n\treturn g.Tx.Query(query, args...)\n}\n\n// Exec Tx")
//line templates/dao_template.go.ego:69
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:69
_, _ = fmt.Fprint(w, " exec\nfunc (g Tx")
//line templates/dao_template.go.ego:70
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:70
_, _ = fmt.Fprint(w, ") Exec(query string, args ...interface{}) (sql.Result, error) {\n\treturn g.Tx.Exec(query, args...)\n}\n\nvar _ ")
//line templates/dao_template.go.ego:74
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:74
_, _ = fmt.Fprint(w, "Queryer = (*Tx")
//line templates/dao_template.go.ego:74
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:74
_, _ = fmt.Fprint(w, ")(nil)\n\n// Tx")
//line templates/dao_template.go.ego:76
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:76
_, _ = fmt.Fprint(w, " is ")
//line templates/dao_template.go.ego:76
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:76
_, _ = fmt.Fprint(w, ".\nfunc Tx")
//line templates/dao_template.go.ego:77
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:77
_, _ = fmt.Fprint(w, "(tx *sql.Tx) Tx")
//line templates/dao_template.go.ego:77
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:77
_, _ = fmt.Fprint(w, " {\n    tblDao := Tx")
//line templates/dao_template.go.ego:78
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:78
_, _ = fmt.Fprint(w, "{}\n    tblDao.Tx = tx\n\ttblDao.TableName = table")
//line templates/dao_template.go.ego:80
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:81
_, _ = fmt.Fprint(w, "\n\ttblDao.Columns = columns")
//line templates/dao_template.go.ego:81
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:82
_, _ = fmt.Fprint(w, "\n\treturn tblDao\n}\n\n// SelectAll select ")
//line templates/dao_template.go.ego:85
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:85
_, _ = fmt.Fprint(w, " table all recode.\nfunc (g ")
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, ") SelectAll() ([]")
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:86
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_template.go.ego:87
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:87
_, _ = fmt.Fprint(w, "SelectAll(g)\n}\n\n// SelectAll transaction select ")
//line templates/dao_template.go.ego:90
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:90
_, _ = fmt.Fprint(w, " table all recode.\nfunc (g Tx")
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, ") SelectAll() ([]")
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:91
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_template.go.ego:92
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:92
_, _ = fmt.Fprint(w, "SelectAll(g)\n}\n\nfunc _")
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, "SelectAll(g ")
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, "Queryer) ([]")
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:95
_, _ = fmt.Fprint(w, ", error) {\n    queryString := queryArgs(\"")
//line templates/dao_template.go.ego:96
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:96
_, _ = fmt.Fprint(w, "\", \"selectAll\", nil)\n\n\tvar es []")
//line templates/dao_template.go.ego:98
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:98
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:98
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:99
_, _ = fmt.Fprint(w, "\n\trows, err := g.Query(queryString)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tdefer rows.Close()\n\n\tfor rows.Next() {\n\t\tvar e ")
//line templates/dao_template.go.ego:106
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:106
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:106
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:107
_, _ = fmt.Fprint(w, "\n\t\tif err := e.Scan(rows); err != nil {\n\t\t\tbreak\n\t\t}\n\n\t\tes = append(es, e)\n\t}\n\tif err != nil {\n\t    log.Println(err, queryString)\n\t\treturn nil, err\n\t}\n\n\treturn es, nil\n}\n\n// SelectByID select ")
//line templates/dao_template.go.ego:121
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:121
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc (g ")
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, ") SelectByID(")
//line templates/dao_template.go.ego:122
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:122
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:122
 continue 
//line templates/dao_template.go.ego:122
 } 
//line templates/dao_template.go.ego:122
 if idx != 0 { 
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:122
 } 
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:122
 } 
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:122
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_template.go.ego:123
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:123
_, _ = fmt.Fprint(w, "SelectByID(g, ")
//line templates/dao_template.go.ego:123
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:123
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:123
 continue 
//line templates/dao_template.go.ego:123
 } 
//line templates/dao_template.go.ego:123
 if idx != 0 { 
//line templates/dao_template.go.ego:123
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:123
 } 
//line templates/dao_template.go.ego:123
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:123
 } 
//line templates/dao_template.go.ego:123
_, _ = fmt.Fprint(w, ")\n}\n\n// SelectByID transaction select ")
//line templates/dao_template.go.ego:126
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:126
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc (g Tx")
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, ") SelectByID(")
//line templates/dao_template.go.ego:127
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:127
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:127
 continue 
//line templates/dao_template.go.ego:127
 } 
//line templates/dao_template.go.ego:127
 if idx != 0 { 
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:127
 } 
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:127
 } 
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:127
_, _ = fmt.Fprint(w, ", error) {\n\treturn _")
//line templates/dao_template.go.ego:128
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:128
_, _ = fmt.Fprint(w, "SelectByID(g, ")
//line templates/dao_template.go.ego:128
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:128
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:128
 continue 
//line templates/dao_template.go.ego:128
 } 
//line templates/dao_template.go.ego:128
 if idx != 0 { 
//line templates/dao_template.go.ego:128
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:128
 } 
//line templates/dao_template.go.ego:128
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:128
 } 
//line templates/dao_template.go.ego:128
_, _ = fmt.Fprint(w, ")\n}\n\nfunc _")
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, "SelectByID(g ")
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, "Queryer, ")
//line templates/dao_template.go.ego:131
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:131
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:131
 continue 
//line templates/dao_template.go.ego:131
 } 
//line templates/dao_template.go.ego:131
 if idx != 0 { 
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:131
 } 
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:131
 } 
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, ") (")
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:131
_, _ = fmt.Fprint(w, ", error) {\n    args := goma.QueryArgs{\n    ")
//line templates/dao_template.go.ego:133
 for _, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:133
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:133
 continue 
//line templates/dao_template.go.ego:133
 } 
//line templates/dao_template.go.ego:133
_, _ = fmt.Fprint(w, "    \"")
//line templates/dao_template.go.ego:133
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_template.go.ego:133
_, _ = fmt.Fprint(w, "\": ")
//line templates/dao_template.go.ego:133
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:133
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_template.go.ego:134
 } 
//line templates/dao_template.go.ego:134
_, _ = fmt.Fprint(w, "}\n\tqueryString := queryArgs(\"")
//line templates/dao_template.go.ego:135
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:135
_, _ = fmt.Fprint(w, "\", \"selectByID\", args)\n\n\trows, err := g.Query(queryString)\n\tif err != nil {\n\t\treturn ")
//line templates/dao_template.go.ego:139
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:139
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:139
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:139
_, _ = fmt.Fprint(w, "{}, err\n\t}\n\tdefer rows.Close()\n\n    if !rows.Next() {\n\t\treturn ")
//line templates/dao_template.go.ego:144
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:144
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:144
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:144
_, _ = fmt.Fprint(w, "{}, sql.ErrNoRows\n\t}\n\n\tvar e ")
//line templates/dao_template.go.ego:147
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:147
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:147
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:148
_, _ = fmt.Fprint(w, "\n\tif err := e.Scan(rows); err != nil {\n\t    log.Println(err, queryString)\n\t\treturn ")
//line templates/dao_template.go.ego:150
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:150
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:150
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:150
_, _ = fmt.Fprint(w, "{}, err\n\t}\n\t\n\treturn e, nil\n}\n\n// Insert insert ")
//line templates/dao_template.go.ego:156
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:156
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, ") Insert(e ")
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:157
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:158
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:158
_, _ = fmt.Fprint(w, "Insert(g, e)\n}\n\n// Insert transaction insert ")
//line templates/dao_template.go.ego:161
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:161
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, ") Insert(e ")
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:162
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:163
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:163
_, _ = fmt.Fprint(w, "Insert(g, e)\n}\n\nfunc _")
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, "Insert(g ")
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, "Queryer, e ")
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:166
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\targs := goma.QueryArgs{\n    ")
//line templates/dao_template.go.ego:168
 for _, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:168
 if !column.IsAutoIncrement { 
//line templates/dao_template.go.ego:168
_, _ = fmt.Fprint(w, " \"")
//line templates/dao_template.go.ego:168
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_template.go.ego:168
_, _ = fmt.Fprint(w, "\": e.")
//line templates/dao_template.go.ego:168
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/dao_template.go.ego:168
_, _ = fmt.Fprint(w, ",")
//line templates/dao_template.go.ego:168
 } 
//line templates/dao_template.go.ego:169
_, _ = fmt.Fprint(w, "\n    ")
//line templates/dao_template.go.ego:169
 } 
//line templates/dao_template.go.ego:170
_, _ = fmt.Fprint(w, "\n\t}\n\tqueryString := queryArgs(\"")
//line templates/dao_template.go.ego:171
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:171
_, _ = fmt.Fprint(w, "\", \"insert\", args)\n\t\n\tresult, err := g.Exec(queryString)\n    if err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n\n// Update update ")
//line templates/dao_template.go.ego:180
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:180
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, ") Update(e ")
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:181
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:182
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:182
_, _ = fmt.Fprint(w, "Update(g, e)\n}\n\n// Update transaction update ")
//line templates/dao_template.go.ego:185
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:185
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, ") Update(e ")
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:186
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:187
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:187
_, _ = fmt.Fprint(w, "Update(g, e)\n}\n\n// Update update ")
//line templates/dao_template.go.ego:190
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:190
_, _ = fmt.Fprint(w, " table.\nfunc _")
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, "Update(g ")
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, "Queryer, e ")
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, ".")
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/dao_template.go.ego:191
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\targs := goma.QueryArgs{\n\t")
//line templates/dao_template.go.ego:193
 for _, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:193
_, _ = fmt.Fprint(w, "  \"")
//line templates/dao_template.go.ego:193
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_template.go.ego:193
_, _ = fmt.Fprint(w, "\": e.")
//line templates/dao_template.go.ego:193
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/dao_template.go.ego:193
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_template.go.ego:194
 } 
//line templates/dao_template.go.ego:195
_, _ = fmt.Fprint(w, "\n\t}\n\tqueryString := queryArgs(\"")
//line templates/dao_template.go.ego:196
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:196
_, _ = fmt.Fprint(w, "\", \"update\", args)\n\n\tresult, err := g.Exec(queryString)\n\tif err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n\n// Delete delete ")
//line templates/dao_template.go.ego:205
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:205
_, _ = fmt.Fprint(w, " table.\nfunc (g ")
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, ") Delete(")
//line templates/dao_template.go.ego:206
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:206
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:206
 continue 
//line templates/dao_template.go.ego:206
 } 
//line templates/dao_template.go.ego:206
 if idx != 0 { 
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:206
 } 
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:206
 } 
//line templates/dao_template.go.ego:206
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:207
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:207
_, _ = fmt.Fprint(w, "Delete(g, ")
//line templates/dao_template.go.ego:207
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:207
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:207
 continue 
//line templates/dao_template.go.ego:207
 } 
//line templates/dao_template.go.ego:207
 if idx != 0 { 
//line templates/dao_template.go.ego:207
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:207
 } 
//line templates/dao_template.go.ego:207
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:207
 } 
//line templates/dao_template.go.ego:207
_, _ = fmt.Fprint(w, ")\n}\n\n// Delete transaction delete ")
//line templates/dao_template.go.ego:210
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:210
_, _ = fmt.Fprint(w, " table.\nfunc (g Tx")
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, ") Delete(")
//line templates/dao_template.go.ego:211
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:211
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:211
 continue 
//line templates/dao_template.go.ego:211
 } 
//line templates/dao_template.go.ego:211
 if idx != 0 { 
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:211
 } 
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:211
 } 
//line templates/dao_template.go.ego:211
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n\treturn _")
//line templates/dao_template.go.ego:212
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:212
_, _ = fmt.Fprint(w, "Delete(g, ")
//line templates/dao_template.go.ego:212
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:212
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:212
 continue 
//line templates/dao_template.go.ego:212
 } 
//line templates/dao_template.go.ego:212
 if idx != 0 { 
//line templates/dao_template.go.ego:212
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:212
 } 
//line templates/dao_template.go.ego:212
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:212
 } 
//line templates/dao_template.go.ego:212
_, _ = fmt.Fprint(w, ")\n}\n\n// Delete delete ")
//line templates/dao_template.go.ego:215
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:215
_, _ = fmt.Fprint(w, " table by primaryKey.\nfunc _")
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, "Delete(g ")
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, "Queryer, ")
//line templates/dao_template.go.ego:216
 for idx, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:216
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:216
 continue 
//line templates/dao_template.go.ego:216
 } 
//line templates/dao_template.go.ego:216
 if idx != 0 { 
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, ", ")
//line templates/dao_template.go.ego:216
 } 
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, " ")
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/dao_template.go.ego:216
 } 
//line templates/dao_template.go.ego:216
_, _ = fmt.Fprint(w, ") (sql.Result, error) {\n    args := goma.QueryArgs{\n    ")
//line templates/dao_template.go.ego:218
 for _, column := range daoData.Table.Columns { 
//line templates/dao_template.go.ego:218
 if !column.IsPrimaryKey { 
//line templates/dao_template.go.ego:218
 continue 
//line templates/dao_template.go.ego:218
 } 
//line templates/dao_template.go.ego:218
_, _ = fmt.Fprint(w, "    \"")
//line templates/dao_template.go.ego:218
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/dao_template.go.ego:218
_, _ = fmt.Fprint(w, "\": ")
//line templates/dao_template.go.ego:218
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.CamelName() )))
//line templates/dao_template.go.ego:218
_, _ = fmt.Fprint(w, ",\n    ")
//line templates/dao_template.go.ego:219
 } 
//line templates/dao_template.go.ego:219
_, _ = fmt.Fprint(w, "}\n\tqueryString := queryArgs(\"")
//line templates/dao_template.go.ego:220
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/dao_template.go.ego:220
_, _ = fmt.Fprint(w, "\", \"delete\", args)\n\n    result, err := g.Exec(queryString)\n\tif err != nil {\n        log.Println(err, queryString)\n    }\n    return result, err\n}\n")
return nil
}
//line templates/delete_template.sql.ego:1
func DeleteTemplate(w io.Writer, tableData TableTemplateData) error  {
//line templates/delete_template.sql.ego:1
_, _ = fmt.Fprint(w, "delete\nfrom\n  ")
//line templates/delete_template.sql.ego:3
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  tableData.Name )))
//line templates/delete_template.sql.ego:4
_, _ = fmt.Fprint(w, "\nwhere\n")
//line templates/delete_template.sql.ego:5
 for idx, column := range tableData.Columns { 
//line templates/delete_template.sql.ego:5
 if !column.IsPrimaryKey { 
//line templates/delete_template.sql.ego:5
 continue 
//line templates/delete_template.sql.ego:5
 } 
//line templates/delete_template.sql.ego:5
 if idx != 0 { 
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, "and")
//line templates/delete_template.sql.ego:5
 } 
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, "  ")
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, " = /* ")
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/delete_template.sql.ego:5
_, _ = fmt.Fprint(w, " */1\n")
//line templates/delete_template.sql.ego:6
 } 
//line templates/delete_template.sql.ego:7
_, _ = fmt.Fprint(w, "\n")
return nil
}
//line templates/entity_template.go.ego:1
func EntityTemplate(w io.Writer, daoData DaoTemplateData) error  {
//line templates/entity_template.go.ego:1
_, _ = fmt.Fprint(w, "package ")
//line templates/entity_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityPkgName )))
//line templates/entity_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\nimport (\n\t\"database/sql\"\n\t")
//line templates/entity_template.go.ego:5
 if daoData.IsDriverPkg() { 
//line templates/entity_template.go.ego:5
_, _ = fmt.Fprint(w, "\"database/sql/driver\"")
//line templates/entity_template.go.ego:5
 } 
//line templates/entity_template.go.ego:6
_, _ = fmt.Fprint(w, "\n\t")
//line templates/entity_template.go.ego:6
 if len(daoData.Imports) > 0 { 
//line templates/entity_template.go.ego:7
_, _ = fmt.Fprint(w, "\n    \t")
//line templates/entity_template.go.ego:7
 for _, importName := range daoData.Imports { 
//line templates/entity_template.go.ego:8
_, _ = fmt.Fprint(w, "\n    \t    \"")
//line templates/entity_template.go.ego:8
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  importName )))
//line templates/entity_template.go.ego:8
_, _ = fmt.Fprint(w, "\"\n        ")
//line templates/entity_template.go.ego:9
 } 
//line templates/entity_template.go.ego:10
_, _ = fmt.Fprint(w, "\n\t")
//line templates/entity_template.go.ego:10
 } 
//line templates/entity_template.go.ego:10
_, _ = fmt.Fprint(w, ")\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\n")
//line templates/entity_template.go.ego:16
 if daoData.EntityBlock == "" { 
//line templates/entity_template.go.ego:17
_, _ = fmt.Fprint(w, "\n// ")
//line templates/entity_template.go.ego:17
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/entity_template.go.ego:17
_, _ = fmt.Fprint(w, " is generated ")
//line templates/entity_template.go.ego:17
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.Name )))
//line templates/entity_template.go.ego:17
_, _ = fmt.Fprint(w, " table.\ntype ")
//line templates/entity_template.go.ego:18
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/entity_template.go.ego:18
_, _ = fmt.Fprint(w, " struct {\n")
//line templates/entity_template.go.ego:19
 for _, column := range daoData.Table.Columns { 
//line templates/entity_template.go.ego:19
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/entity_template.go.ego:19
_, _ = fmt.Fprint(w, " ")
//line templates/entity_template.go.ego:19
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TypeName )))
//line templates/entity_template.go.ego:19
_, _ = fmt.Fprint(w, " ")
//line templates/entity_template.go.ego:19
_, _ = fmt.Fprintf(w, "%v",  column.TypeDetail )
//line templates/entity_template.go.ego:20
_, _ = fmt.Fprint(w, "\n")
//line templates/entity_template.go.ego:20
 } 
//line templates/entity_template.go.ego:20
_, _ = fmt.Fprint(w, "}\n")
//line templates/entity_template.go.ego:21
 } else { 
//line templates/entity_template.go.ego:22
_, _ = fmt.Fprint(w, "\n")
//line templates/entity_template.go.ego:22
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityBlock )))
//line templates/entity_template.go.ego:23
_, _ = fmt.Fprint(w, "\n")
//line templates/entity_template.go.ego:23
 } 
//line templates/entity_template.go.ego:24
_, _ = fmt.Fprint(w, "\n\n// Scan ")
//line templates/entity_template.go.ego:25
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/entity_template.go.ego:25
_, _ = fmt.Fprint(w, " all scan\nfunc (e *")
//line templates/entity_template.go.ego:26
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.EntityName )))
//line templates/entity_template.go.ego:26
_, _ = fmt.Fprint(w, ") Scan(rows *sql.Rows) error {\n\treturn rows.Scan(")
//line templates/entity_template.go.ego:27
 for idx, column := range daoData.Table.Columns { 
//line templates/entity_template.go.ego:27
 if idx != 0 { 
//line templates/entity_template.go.ego:27
_, _ = fmt.Fprint(w, ", ")
//line templates/entity_template.go.ego:27
 } 
//line templates/entity_template.go.ego:27
_, _ = fmt.Fprint(w, "&e.")
//line templates/entity_template.go.ego:27
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.TitleName )))
//line templates/entity_template.go.ego:27
 } 
//line templates/entity_template.go.ego:27
_, _ = fmt.Fprint(w, ")\n}\n\n")
//line templates/entity_template.go.ego:30
 for _, column := range daoData.Table.Columns { 
//line templates/entity_template.go.ego:31
_, _ = fmt.Fprint(w, "\n\t")
//line templates/entity_template.go.ego:31
 if column.EnumData.TypeName != "" { 
//line templates/entity_template.go.ego:32
_, _ = fmt.Fprint(w, "\n\t\t// ")
//line templates/entity_template.go.ego:32
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:32
_, _ = fmt.Fprint(w, " ")
//line templates/entity_template.go.ego:32
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:32
_, _ = fmt.Fprint(w, " column type\n\t\ttype ")
//line templates/entity_template.go.ego:33
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:33
_, _ = fmt.Fprint(w, " string\n\n\t\t// ")
//line templates/entity_template.go.ego:35
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, "\n\t\tconst (")
//line templates/entity_template.go.ego:36
 for _, e := range column.EnumData.Enums { 
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  e.Name )))
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, " ")
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, " = \"")
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  e.Value )))
//line templates/entity_template.go.ego:36
_, _ = fmt.Fprint(w, "\"\n\t\t")
//line templates/entity_template.go.ego:37
 } 
//line templates/entity_template.go.ego:37
_, _ = fmt.Fprint(w, ")\n\n\t\t// Scan database/sql Scanner\n\t\tfunc (e *")
//line templates/entity_template.go.ego:40
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:40
_, _ = fmt.Fprint(w, ") Scan(v interface{}) error {\n        \t*e = ")
//line templates/entity_template.go.ego:41
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:41
_, _ = fmt.Fprint(w, "(v.([]byte))\n        \treturn nil\n        }\n\n        var _ sql.Scanner = (*")
//line templates/entity_template.go.ego:45
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:45
_, _ = fmt.Fprint(w, ")(nil)\n\n        // Value database/sql/driver Valuer\n        func (e ")
//line templates/entity_template.go.ego:48
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:48
_, _ = fmt.Fprint(w, ") Value() (driver.Value, error) {\n        \treturn string(e), nil\n        }\n\n        var _ driver.Valuer = (*")
//line templates/entity_template.go.ego:52
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.EnumData.TypeName )))
//line templates/entity_template.go.ego:52
_, _ = fmt.Fprint(w, ")(nil)\n\t")
//line templates/entity_template.go.ego:53
 } 
//line templates/entity_template.go.ego:54
_, _ = fmt.Fprint(w, "\n")
//line templates/entity_template.go.ego:54
 } 
return nil
}
//line templates/gomautils_template.go.ego:1
func HelperTemplate(w io.Writer, helperData HelperTemplateData) error  {
//line templates/gomautils_template.go.ego:1
_, _ = fmt.Fprint(w, "package ")
//line templates/gomautils_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.PkgName )))
//line templates/gomautils_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n    ")
//line templates/gomautils_template.go.ego:8
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.DriverImport )))
//line templates/gomautils_template.go.ego:9
_, _ = fmt.Fprint(w, "\n\t\n\t\"")
//line templates/gomautils_template.go.ego:10
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.DaoImport )))
//line templates/gomautils_template.go.ego:10
_, _ = fmt.Fprint(w, "\"\n        \n\t\"github.com/kyokomi/goma\"\n)\n\n// Goma goma.Goma utils.\ntype Goma struct {\n\t*goma.Goma\n}\n\n// NewGoma is goma.Goma wrapper utils.\nfunc NewGoma(configPath string) (Goma, error) {\n    opts, err := goma.NewOptions(configPath)\n    if err != nil {\n        return Goma{}, err\n    }\n    g, err := goma.NewGoma(opts)\n    if err != nil {\n        return Goma{}, err\n    }\n\n    gm := Goma{}\n    gm.Goma = g\n\treturn gm, nil\n}\n")
//line templates/gomautils_template.go.ego:35
 for _, daoData := range helperData.DaoList { 
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, "\n// ")
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, " is ")
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.DaoPkgName )))
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, ".")
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/gomautils_template.go.ego:36
_, _ = fmt.Fprint(w, " helper.\nfunc (g Goma) ")
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, "() ")
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.DaoPkgName )))
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, ".")
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Name )))
//line templates/gomautils_template.go.ego:37
_, _ = fmt.Fprint(w, " {\n    return ")
//line templates/gomautils_template.go.ego:38
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  helperData.DaoPkgName )))
//line templates/gomautils_template.go.ego:38
_, _ = fmt.Fprint(w, ".")
//line templates/gomautils_template.go.ego:38
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  daoData.Table.TitleName )))
//line templates/gomautils_template.go.ego:38
_, _ = fmt.Fprint(w, "(g.Goma)\n}\n")
//line templates/gomautils_template.go.ego:40
 } 
//line templates/gomautils_template.go.ego:41
_, _ = fmt.Fprint(w, "\n")
return nil
}
//line templates/insert_template.sql.ego:1
func InsertTemplate(w io.Writer, tableData TableTemplateData) error  {
//line templates/insert_template.sql.ego:1
_, _ = fmt.Fprint(w, "insert into ")
//line templates/insert_template.sql.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  tableData.Name )))
//line templates/insert_template.sql.ego:1
_, _ = fmt.Fprint(w, "(\n  ")
//line templates/insert_template.sql.ego:2
 var isComma bool 
//line templates/insert_template.sql.ego:2
 isComma = false 
//line templates/insert_template.sql.ego:2
 for _, column := range tableData.Columns { 
//line templates/insert_template.sql.ego:2
 if !column.IsAutoIncrement { 
//line templates/insert_template.sql.ego:2
 if !isComma { 
//line templates/insert_template.sql.ego:2
 isComma = true 
//line templates/insert_template.sql.ego:2
 } else { 
//line templates/insert_template.sql.ego:2
_, _ = fmt.Fprint(w, ", ")
//line templates/insert_template.sql.ego:2
 } 
//line templates/insert_template.sql.ego:2
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/insert_template.sql.ego:2
 } 
//line templates/insert_template.sql.ego:3
_, _ = fmt.Fprint(w, "\n")
//line templates/insert_template.sql.ego:3
 } 
//line templates/insert_template.sql.ego:3
_, _ = fmt.Fprint(w, ") values(\n  ")
//line templates/insert_template.sql.ego:4
 isComma = false 
//line templates/insert_template.sql.ego:4
 for _, column := range tableData.Columns { 
//line templates/insert_template.sql.ego:4
 if !column.IsAutoIncrement { 
//line templates/insert_template.sql.ego:4
 if !isComma { 
//line templates/insert_template.sql.ego:4
 isComma = true 
//line templates/insert_template.sql.ego:4
 } else { 
//line templates/insert_template.sql.ego:4
_, _ = fmt.Fprint(w, ", ")
//line templates/insert_template.sql.ego:4
 } 
//line templates/insert_template.sql.ego:4
_, _ = fmt.Fprint(w, "/* ")
//line templates/insert_template.sql.ego:4
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/insert_template.sql.ego:4
_, _ = fmt.Fprint(w, " */")
//line templates/insert_template.sql.ego:4
_, _ = fmt.Fprintf(w, "%v",  column.Sample )
//line templates/insert_template.sql.ego:4
 } 
//line templates/insert_template.sql.ego:5
_, _ = fmt.Fprint(w, "\n")
//line templates/insert_template.sql.ego:5
 } 
//line templates/insert_template.sql.ego:5
_, _ = fmt.Fprint(w, ")\n")
return nil
}
//line templates/queryargs_template.go.ego:1
func QueryArgsTemplate(w io.Writer, queryArgsData QueryArgsTemplateData) error  {
//line templates/queryargs_template.go.ego:1
_, _ = fmt.Fprint(w, "package  ")
//line templates/queryargs_template.go.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  queryArgsData.DaoPkgName )))
//line templates/queryargs_template.go.ego:2
_, _ = fmt.Fprint(w, "\n\n// NOTE: THIS FILE WAS PRODUCED BY THE\n// GOMA CODE GENERATION TOOL (github.com/kyokomi/goma)\n// DO NOT EDIT\n\nimport (\n\t\"path/filepath\"\n\n\t\"github.com/kyokomi/goma\"\n)\n\ntype queryArgSettings struct {\n\trootDir string\n\tsqlFile bool\n}\n\nvar settings queryArgSettings\n\n// SetupQueryArgs setup query file path\nfunc SetupQueryArgs(rootDir string, sqlFile bool) {\n\tsettings.rootDir = rootDir\n\tsettings.sqlFile = sqlFile\n}\n\n// GenerateQuery exported queryArgs\nfunc GenerateQuery(tableName string, queryName string, args goma.QueryArgs) string {\n\treturn queryArgs(tableName, queryName, args)\n}\n\nfunc queryArgs(tableName string, queryName string, args goma.QueryArgs) string {\n\treturn settings.queryArgs(tableName, queryName, args)\n}\n\nfunc (s queryArgSettings) queryArgs(tableName string, queryName string, args goma.QueryArgs) string {\n\tfilePath := createSqlFilePath(s.rootDir, tableName, queryName)\n\t")
//line templates/queryargs_template.go.ego:37
 if queryArgsData.DriverName == "mysql" { 
//line templates/queryargs_template.go.ego:37
_, _ = fmt.Fprint(w, "return goma.MySQLGenerateQuery(assetSQL(filePath), args)\n\t")
//line templates/queryargs_template.go.ego:38
 } else if queryArgsData.DriverName == "postgres" { 
//line templates/queryargs_template.go.ego:38
_, _ = fmt.Fprint(w, "return goma.PostgresGenerateQuery(assetSQL(filePath), args)\n\t")
//line templates/queryargs_template.go.ego:39
 } else { 
//line templates/queryargs_template.go.ego:39
_, _ = fmt.Fprint(w, "return \"error\"\n")
//line templates/queryargs_template.go.ego:40
 } 
//line templates/queryargs_template.go.ego:40
_, _ = fmt.Fprint(w, "}\n\nfunc assetSQL(filePath string) string {\n\tvar data []byte\n\tvar err error\n\tif settings.sqlFile {\n\t\tdata, err = AssetFile(filePath)\n\t} else {\n\t\tdata, err = Asset(filePath)\n\t}\n\tif err != nil {\n\t\t// Asset was not found.\n\t}\n\treturn string(data)\n}\n\nfunc createSqlFilePath(rootDir string, tableName string, queryName string) string {\n\treturn filepath.Join(rootDir, \"")
//line templates/queryargs_template.go.ego:57
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  queryArgsData.SQLRootDir )))
//line templates/queryargs_template.go.ego:57
_, _ = fmt.Fprint(w, "\", tableName, queryName+\".sql\")\n}\n")
return nil
}
//line templates/selectAll_template.sql.ego:1
func SelectAllTemplate(w io.Writer, tableData TableTemplateData) error  {
//line templates/selectAll_template.sql.ego:1
_, _ = fmt.Fprint(w, "select\n  ")
//line templates/selectAll_template.sql.ego:2
 for idx, column := range tableData.Columns { 
//line templates/selectAll_template.sql.ego:2
 if idx != 0 { 
//line templates/selectAll_template.sql.ego:2
_, _ = fmt.Fprint(w, ", ")
//line templates/selectAll_template.sql.ego:2
 } 
//line templates/selectAll_template.sql.ego:2
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/selectAll_template.sql.ego:3
_, _ = fmt.Fprint(w, "\n")
//line templates/selectAll_template.sql.ego:3
 } 
//line templates/selectAll_template.sql.ego:3
_, _ = fmt.Fprint(w, "FROM\n  ")
//line templates/selectAll_template.sql.ego:4
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  tableData.Name )))
//line templates/selectAll_template.sql.ego:5
_, _ = fmt.Fprint(w, "\n")
return nil
}
//line templates/selectByID_template.sql.ego:1
func SelectByIDTemplate(w io.Writer, tableData TableTemplateData) error  {
//line templates/selectByID_template.sql.ego:1
_, _ = fmt.Fprint(w, "select\n  ")
//line templates/selectByID_template.sql.ego:2
 for idx, column := range tableData.Columns { 
//line templates/selectByID_template.sql.ego:2
 if idx != 0 { 
//line templates/selectByID_template.sql.ego:2
_, _ = fmt.Fprint(w, ", ")
//line templates/selectByID_template.sql.ego:2
 } 
//line templates/selectByID_template.sql.ego:2
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/selectByID_template.sql.ego:3
_, _ = fmt.Fprint(w, "\n")
//line templates/selectByID_template.sql.ego:3
 } 
//line templates/selectByID_template.sql.ego:3
_, _ = fmt.Fprint(w, "FROM\n  ")
//line templates/selectByID_template.sql.ego:4
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  tableData.Name )))
//line templates/selectByID_template.sql.ego:5
_, _ = fmt.Fprint(w, "\nWHERE\n")
//line templates/selectByID_template.sql.ego:6
 for idx, column := range tableData.Columns { 
//line templates/selectByID_template.sql.ego:6
 if !column.IsPrimaryKey { 
//line templates/selectByID_template.sql.ego:6
 continue 
//line templates/selectByID_template.sql.ego:6
 } 
//line templates/selectByID_template.sql.ego:6
 if idx != 0 { 
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, "and")
//line templates/selectByID_template.sql.ego:6
 } 
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, "  ")
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, " = /* ")
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, " */")
//line templates/selectByID_template.sql.ego:6
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Sample )))
//line templates/selectByID_template.sql.ego:7
_, _ = fmt.Fprint(w, "\n")
//line templates/selectByID_template.sql.ego:7
 } 
//line templates/selectByID_template.sql.ego:8
_, _ = fmt.Fprint(w, "\n")
return nil
}
//line templates/update_template.sql.ego:1
func UpdateTemplate(w io.Writer, tableData TableTemplateData) error  {
//line templates/update_template.sql.ego:1
_, _ = fmt.Fprint(w, "update ")
//line templates/update_template.sql.ego:1
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  tableData.Name )))
//line templates/update_template.sql.ego:1
_, _ = fmt.Fprint(w, " set\n  ")
//line templates/update_template.sql.ego:2
 for idx, column := range tableData.Columns { 
//line templates/update_template.sql.ego:2
 if idx != 0 { 
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, ", ")
//line templates/update_template.sql.ego:2
 } 
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, "  ")
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, " = /* ")
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprint(w, " */")
//line templates/update_template.sql.ego:2
_, _ = fmt.Fprintf(w, "%v",  column.Sample )
//line templates/update_template.sql.ego:3
_, _ = fmt.Fprint(w, "\n")
//line templates/update_template.sql.ego:3
 } 
//line templates/update_template.sql.ego:3
_, _ = fmt.Fprint(w, " where\n  ")
//line templates/update_template.sql.ego:4
 for idx, column := range tableData.Columns { 
//line templates/update_template.sql.ego:4
 if !column.IsPrimaryKey { 
//line templates/update_template.sql.ego:4
 continue 
//line templates/update_template.sql.ego:4
 } 
//line templates/update_template.sql.ego:4
 if idx != 0 { 
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, "and")
//line templates/update_template.sql.ego:4
 } 
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, "  ")
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, " = /* ")
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, html.EscapeString(fmt.Sprintf("%v",  column.Name )))
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprint(w, " */")
//line templates/update_template.sql.ego:4
_, _ = fmt.Fprintf(w, "%v",  column.Sample )
//line templates/update_template.sql.ego:5
_, _ = fmt.Fprint(w, "\n")
//line templates/update_template.sql.ego:5
 } 
//line templates/update_template.sql.ego:6
_, _ = fmt.Fprint(w, "\n")
return nil
}
